...
int[,] SortLargestSmallestElementLineMatrix(int[,] matrix)
{
    int buffer = 0;

    for (int i = 0; i < matrix.GetLength(0); i++)
    {
        matrix = BubbleSortForSortLargestSmallestElementLineMatrix(line:i, matrix);
    }
    return matrix;
}

int[,] BubbleSortForSortLargestSmallestElementLineMatrix(int[,] matrix, int line)
{
        int buffer = 0;
        ...
        for (int j = 0; j < matrix.GetLength(1); j++)
        {
            ...
            {
                if (matrix[line, j] < matrix[line, l])
                {
                    buffer = matrix[line, j];
                    matrix[line, j] = matrix[line, l];
                    matrix[line, l] = buffer;
                }
            }
        }
        return matrix;
    ...
    )
{
    ...
    int[,] SortMatrixRandomInt = SortLargestSmallestElementLineMatrix(MatrixRandomInt);
