...
double ColumnSumma(int[,] matrix, int column)
{
        double ColumnSummaElement = 0;

        for (int i = 0; i < matrix.GetLength(0); i++)
        {
            ColumnSummaElement = ColumnSummaElement + matrix[i, column];
        }

        return ColumnSummaElement;
}

double[] SearchForAllColumnSumma(int[,] matrix)
{
    double[] ColumnSummaAllElement = new double[matrix.GetLength(1)];

    for (int j = 0; j < matrix.GetLength(1); j++)
    {
        ColumnSummaAllElement[j] = ColumnSumma(matrix , column:j);
    }
    return ColumnSummaAllElement;
}
    ...
    )
{
    ...
    double[] ColumnSummaElement = SearchForAllColumnSumma(MatrixRandomInt);
    
    PrintConsoleDoubleArray(ColumnSummaElement);

    System.Console.WriteLine("Cреднее арифметическое элементов для каждого столбца сгенерированной матрицы:");
    
    double[] ColumnAverage = SearchColumnAverage(MatrixRandomInt, ColumnSummaElement);
